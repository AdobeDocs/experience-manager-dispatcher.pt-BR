...
1. Se os arquivos armazenados em cache não forem válidos, o Dispatcher solicitará páginas recém-renderizadas da instância de publicação do AEM.
...
* As instâncias de criação e publicação do AEM são implantadas em sistemas Linux®.
...
Com essa configuração, o servidor web executa as seguintes ações ao receber uma solicitação para `https://branda.com/en/products.html`:
...
>[!NOTE]
>
>No AEM as a Cloud Service, uma configuração de vhost separada deve ser usada com um DocumentRoot em um nível mais alto do que cada uma das subpáginas. Isso é tratado por padrão no arquétipo, mas quando várias DocumentRoots são usadas, uma configuração de vhost de prioridade mais alta deve ser usada para que a invalidação do cache possa ser tratada para todo o cache, pois ela não pode ser configurada separadamente para cada site. O ServerAlias dessa nova configuração deve aceitar o cabeçalho de host “localhost”.
...
Além disso, a instância de publicação deve ser designada como o renderizador do host virtual. Configure outras propriedades do farm, conforme necessário. O código a seguir é uma configuração abreviada para o farm usado para invalidar o cache:
...
Depois de criar o mapeamento da página de conteúdo, para descobrir se são necessários mapeamentos adicionais, use um navegador da web para abrir uma página no servidor web. No arquivo error.log da instância de publicação, localize mensagens sobre recursos que não foram encontrados. A seguinte mensagem de exemplo indica que é necessário um mapeamento para `/etc/clientlibs`:
...
>[!NOTE]
>
>O transformador Linkchecker do reescritor padrão do Apache Sling modifica automaticamente os hiperlinks na página para evitar links quebrados. No entanto, a reescrita de links é executada somente quando o destino do link é um arquivo HTML ou HTM. Para atualizar links para outros tipos de arquivo, crie um componente de transformador e adicione-o a um pipeline de reescrita HTML.
...
* A propriedade `DispatcherUseProcessedURL` é definida como `1`. (Linha 10)
...
Para criar um componente transformador e usá-lo em um pipeline, execute as seguintes tarefas:
...
1. Implemente a interface `org.apache.sling.rewriter.Transformer`. Para minimizar o trabalho, você pode estender a classe `org.apache.cocoon.xml.sax.AbstractSAXPipe`. Para personalizar o comportamento do reescritor, substitua o método startElement. Esse método é chamado para cada evento SAX transmitido para o transformador.
1. Agrupar e implantar as classes.
1. Para adicionar o transformador ao pipeline, adicione um nó de configuração no aplicativo do AEM.

>[!TIP]
>Em vez disso, você pode configurar o TransformerFactory para que o transformador seja inserido em cada reescritor definido. Com isso, não é necessário configurar um pipeline:
>
...
>[!NOTE]
>
>Para criar um projeto Maven, use o arquétipo [multimódulo](https://experienceleague.adobe.com/pt-br/docs/experience-manager-release-information/aem-release-updates/previous-updates/aem-previous-versions) do plug-in Maven para pacotes de conteúdo. Os POMs criam e instalam automaticamente um pacote de conteúdo.
